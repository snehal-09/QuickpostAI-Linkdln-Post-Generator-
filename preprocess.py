...
def clean_text(text: str) -> str:
    """
    Removes or replaces surrogate characters (invalid emojis)
    so that UTF-8 encoding doesn't break.
    """
...
def process_posts(raw_file_path, processed_file_path=None):
    with open(raw_file_path, encoding="utf-8") as file:
        ...
        enriched_posts = []
        for post in posts:
            clean_post_text = clean_text(post["text"])  # clean text before sending
    ...
    # ✅ Save processed posts safely (ignores broken emojis)
    with open(processed_file_path, mode="w", encoding="utf-8", errors="ignore") as outfile:
